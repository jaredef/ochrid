...
const NsToggle = () => {
    ...
    try {
        ...
        localStorage.setItem('newCalendar', 'false');

        // Read the current value of 'newCalendar' and log to console
        const currentLocalStorage = localStorage.getItem('newCalendar');
        console.log('newCalendar has been set to:', currentLocalStorage);
    } catch(e) {
        console.error('This session does not support local storage:', e);
    }
    ...
    const handleToggleSwitchClick = () => {
        ...
        localStorage.setItem('newCalendar', isNSActive ? 'false' : 'true');
        if (!isNSActive) {
            handleNSButtonClick();
        } else {
            handleOSButtonClick();
